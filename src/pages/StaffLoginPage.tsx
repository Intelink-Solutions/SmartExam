...
export default function StaffLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const allowedRoles: ApiUserRole[] = role === "teacher" ? ["teacher"] : ["admin", "super_admin"];
      await login(email.trim(), password, allowedRoles);
      navigate("/dashboard");
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-primary relative overflow-hidden">
      ...
      >
        <button
          ...
          aria-label="Back to Student login"
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Admin and Teacher Login</p>
        </div>
